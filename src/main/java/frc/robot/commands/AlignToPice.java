...
public class AlignToPice extends CommandBase {
	...
	private static final Transform3d piceToGoal = new Transform3d(
			new Translation3d(1.5, 0.0, 0.0),
			new Rotation3d(0.0, 0.0, Math.PI));

	public AlignToPice(Swerve swerve, PhotonCamera rightCamera, PhotonCamera leftCamera, PoseEstimator poseEstimator) {
		this.swerve = swerve;
	...
	public void execute() {
		...
		Pose3d robotPose = new Pose3d(
				...
				robotPose2d.getY(),
				0.0,
				new Rotation3d(swerve.getRoll().getRadians(), swerve.getPitch().getRadians(),
		...
		if (photonRes.hasTargets()) {
			...
			Optional<PhotonTrackedTarget> targetOpt = photonRes.getTargets().stream()
					...
					.filter(target -> !target.equals(lastTarget) && target.getPoseAmbiguity() <= .2
			...
			if (targetOpt.isPresent()) {
				...
				Pose2d goalPose = targetPose.transformBy(piceToGoal).toPose2d();
